...
def get_sidechain_ic(xyz, top):
    traj = md.Trajectory(xyz.numpy(), top)
    table, _ = top.to_dataframe()
    # n_res = table.resSeq.max()
    ...
    for i in range(1,n_res-1):
        # rest = table.loc[table.resSeq==i+1]
        rest = table.loc[table.resSeq==res_list[i]]
        
        if len(rest) > 0: 
            ic_i = np.zeros((10, 3))
            resn = rest.resName.values[0]
            # atom_idx = rest.loc[rest.name.isin(atom_list)].index
            # atom_list = ['O', 'C', 'CA'] + core_sc[resn] 
...
def get_backbone_ic(xyz, top):
    ...
    C_torsion = md.compute_dihedrals(traj, np.stack((C[1:-1], CA[1:-1], CA[2:], CA[:-2]), axis=-1))
    N_torsion = md.compute_dihedrals(traj, np.stack((N[1:-1], CA[1:-1], CA[:-2], CA[2:]), axis=-1))
    O_torsion = md.compute_dihedrals(traj, np.stack((O[1:-1], C[1:-1], CA[1:-1], N[1:-1]), axis=-1))

    C_angle = md.compute_angles(traj, np.stack((C[1:-1], CA[1:-1], CA[2:]), axis=-1))
    N_angle = md.compute_angles(traj, np.stack((N[1:-1], CA[1:-1], CA[:-2]), axis=-1))
    O_angle = md.compute_angles(traj, np.stack((O[1:-1], C[1:-1], CA[1:-1]), axis=-1))

    C_dist = md.compute_distances(traj, np.stack((C[1:-1], CA[1:-1]), axis=-1))
    N_dist = md.compute_distances(traj, np.stack((N[1:-1], CA[1:-1]), axis=-1))
    O_dist = md.compute_distances(traj, np.stack((O[1:-1], C[1:-1]), axis=-1))

    C_ic = np.stack((C_dist, C_angle, C_torsion), axis=-1)
    N_ic = np.stack((N_dist, N_angle, N_torsion), axis=-1)
    O_ic = np.stack((O_dist, O_angle, O_torsion), axis=-1)
    
    all_ic = np.stack((N_ic, C_ic, O_ic), axis=2).reshape(-1, 3, 3)
    return all_ic  
...
def ic_to_xyz(CG_nxyz, ic_recon, info):
    ...
    # reslist = list(set(list(top_table.resSeq)))
    # reslist.sort()
    # atomn = [list(top_table.loc[top_table.resSeq==res].name) for res in reslist][1:-1]
    # resn = list(top_table.loc[top_table.name=='CA'].resName)[1:-1]
