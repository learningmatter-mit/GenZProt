...
def CG_collate(dicts):
    ...
    if 'num_hyper_CGs' in dicts[0]:
        cumulative_hyper_CGs = np.cumsum([0] + [d['num_hyper_CGs'] for d in dicts])[:-1]
        for n, d in zip(cumulative_hyper_CGs, dicts):
            d['hyper_CG_mapping'] = d['hyper_CG_mapping'] + int(n)
            d['hyper_CG_nbr_list'] = d['hyper_CG_nbr_list'] + int(n) 
            d['hyper_bond_edge_list'] = d['hyper_bond_edge_list'] + int(n)
