...
# from .sidechain import THREE_LETTER_TO_ONE, RES2IDX
...
def build_ic_multiprotein_dataset(mapping, traj, atom_cutoff, cg_cutoff, atomic_nums, top, order=1, cg_traj=None, prot_idx=None):
    ...
    # indices = top.select_atom_indices('alpha')
    ...
    for nxyz in nxyz_data:
        bb_ic = torch.Tensor(get_backbone_ic(nxyz[:, 1:], top))
        sc_ic = torch.Tensor(get_sidechain_ic(nxyz[:, 1:], top))
        ic = torch.cat((bb_ic, sc_ic), axis=1)
        ic_list.append(ic)
    ...
    # sc_ic = torch.Tensor(get_sidechain_ic(nxyz[:, 1:], top))
    # ic = torch.cat((bb_ic, sc_ic), axis=1)
    # ic_list = [ic for _ in range(len(nxyz_data))]
    ...
    # CG_mapping_list = [torch.LongTensor(mapping[4:-4]) for mapping in CG_mapping_list]

    # nres = num_CGs_list[0]+2
